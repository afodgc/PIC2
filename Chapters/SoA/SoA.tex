...
\section{MPPT Algorithms}
    ...
    \subsection{Perturb and Observe (P\&O)}
        ...
        To address this limitation, an additional condition is incorporated into the algorithm that evaluates two consecutive measurements of $\Delta P$ and $\Delta V$. Specifically, when the signs of consecutive $\Delta P$ measurements differ, this indicates that the observed power variation results from environmental disturbances rather than the algorithm's perturbation; consequently, the voltage adjustment is suppressed. This refined variant is designated the two-point algorithm or improved P\&O method~\cite{bendib_survey_mppt_methods}. Table~\ref{tab:improved-peo-truth-extended} presents the sixteen distinct operational states encompassed by this algorithm's truth table, with an additional column delineating whether observed changes originate from algorithm perturbation or environmental variation.
