...
In this chapter is explained the main state-of-art concepts about the engineering behind an \ac{mppt}. More specifically explains this key concepts: what is a solar panels, how to convert there energy to useful energy, \ac{mppt} algorithms, dc-dc types and battery charging techniques. 


\section{Solar Panels}

    Solar panels, also know as \ac{pv} panels are devices that convert sunlight into electrical energy. Each solar panel is made of multiple solar cells connected in series and/or parallel. As in electrical circuits, the connection type will affect the voltage and current output of the panel. More specifically, series increases voltage and parallel increases current.
    ...
    \begin{figure} [!h]
        \centering
        \includegraphics[width=0.60 \textwidth]{Images/solar_cell.pdf}
        \qquad
    ...
    \begin{figure} [!h]
        \centering
        \includegraphics[width=0.55 \textwidth]{Images/i-v,p-v.pdf}
        \qquad
    ...
    As you can see in Figure~\ref{fig:irradiance_i-v}, the variation of the irradiance change the I-V and P-V curve of the solar panel. The increase of irradiance produces more more power by mainly increasing the current output of the panel. On the other hand, the temperature has an opposite effect, Figure~\ref{fig:temperature_i-v}. The increase of temperature produces a decrease in power output, mainly by decreasing the voltage output of the panel. 
...
\section{Solar energy production}
    As discussed in the previous section, solar panels produce a variable DC voltage and current output which depends on environmental conditions. To extract as much energy as possible, commonly an \ac{mppt} are used. 

    An \acf{mppt} is a device that is used to optimize the power output from solar panels by continuously tracking and adjusting the operation point of the panels to ensure they operate at their \ac{mpp}. 

    To achieve it's goal, an \ac{mppt} is usually composed by an DC-DC converter, an microcontroller and some sensors (Figure \ref{fig:block diagram mppt}). The sensors are used to measure the input variables of the control system (typically voltage and current). The information acquired by this sensors are processed by the microcontroller, which runs an algorithm to determine if the \ac{mpp} was reached or how to act on the system towards the \ac{mpp}. In the second case, the microcontroller sends control signals to the DC-DC converter to adjust its operation accordingly. Usually with PWM to control transistors.
    ...
    Than, the output can be connected to different types of loads, like batteries, DC loads or inverters to convert the energy to alternative current and inject it in the grid (usually in this cases the \ac{mppt} is built-in with the inverter).

\section{MPPT Algotithms}
    ...
    \subsection{Constant Voltage}
        This is the simplest and the most inefficient. This method uses a fixed reference voltage to operate the solar panel. This voltage enters in a PID controller that will adjust the duty cycle of the DC-DC converter to maintain the solar panel voltage at this reference value.

        There are some well know variations of this method that improve the performance by adjusting the reference voltage to a fraction of the open circuit voltage of the solar panel (usually between 71\% and 78\%)~\cite{Comparison_PV_Array_MPPT_Techniques_2007}, Equation \ref{eq:cv_vref}. This open circuit voltage can be measured periodically by disconnecting the solar panel from the load for a short period of time. This way, the reference voltage will be more accurate and the efficiency will increase.
        ...
        Both methods are simple and low cost, but they are inefficient, since they have to stop producing energy to do measurements. But there is also some variations that solve this problem by using a dummy cell to do the measurements or even an diode since its there physical properties are similar to a solar cell \cite{Comparison_PV_Array_MPPT_Techniques_2007} \cite{Optimum_Operating_Point_Tracker_2004}.
    ...
    \subsection{Perturb and Observe (P\&O)}
        ...
        To solve this issue, a new conditions must be added to the algorithm to take into account two consecutive measures of $\Delta P$ and $\Delta V$. This way, if the sign of both $\Delta P$ are different, it means that the power change was caused by an environmental variation and not by the perturbation, so the algorithm must not change the voltage. This variation is called "two point algorithm" or "improved P\&O" \cite{bendib_survey_mppt_methods}. 
        Table~\ref{tab:improved-peo-truth-extended} shows the 16 possible cases of the truth table of this algorithm with an extra column showing the changes caused by the perturbation or by the environment. \textcolor{red}{Por acabar e rever (não sei se gosto muito desta tabela e explicação)}

        \begin{table}[!h]
        \centering
        \caption{Truth table for the Improved P\&O algorithm (extended with one extra column) \cite{bendib_survey_mppt_methods}.}
        \label{tab:improved-peo-truth-extended}
        \newcommand{\pl}{$+$}
        \newcommand{\mi}{$-$}
        \begin{tabular}{cccccc}
            \toprule
            $\Delta V(k-1)$ & $\Delta P(k-1)$ & $\Delta V(k)$ & $\Delta P(k)$ & $\Delta V(k+1)$ & Changes caused by: \\ 
            \midrule
            \mi & \mi & \mi & \mi & \mi & \\ 
            \mi & \mi & \mi & \pl & \pl & \\ 
            \mi & \mi & \pl & \mi & \mi & \\ 
        ...
        There are some more relevant variations of this algorithm that will not be explained here, like the Variable Step Size P\&O (VSS-P\&O), the three point P\&O, a-factor P\&O and others.
    ...
    \subsection{Incremental Conductance (IncCond)}
        ...
        This algorithm lies on the fact that at the \ac{mpp} the derivative of power with respect to voltage is zero. By knowing the output voltage and current of the solar panel, the algorithm can calculate the cundactance and the incremental conductance. 
        ...
        \subsection{Look Up Table (LUT)}
            ...
            The LUT table contains several entries organized by voltage and current. Each of this entries contains the optimal duty cycle of the DC-DC converter, that are pre calculated or measured for a specific voltage and current. This way, the algorithm can reach the mpp in one clock cycle by measuring the voltage and current of the solar panel, looking for the closest entry in the LUT table and applying the corresponding duty cycle to the converter.
...
\section{DC-DC types}
    ...
    \subsection{Non-isolated DC-DC converters}
        Modern non-isolated conversion circuits generally use one of three basic topologies: buck, boost or buck-boost converters. They are 'basic' in the sense that only one switch is used and there is no isolation from the output circuits. A given topology is used to obtain a specific result, such as voltage step-down, voltage step-up, or hybrid mode \cite{Comparison_PV_Converter_Technologies_1989}. 

        \subsubsection{Boost converter}
            The boost converter is a step-up DC-DC converter that is widely used in \ac{mppt} because solar panels usually have a lower voltage than the battery or load they are powering. There are multiple papers that use this topology \textcolor{red}{buscar alguns}.

            Also this DC-DC topologies are simple to implement, low cost and have high efficiency, reaching efficiencies of up 98\% \cite{Review_Isolated_Non_isolated_DC_DC_Converter_PV_Application_2018}. 

            One down side of this topology is that can not emulate smaller impedances than the load impedance, and therefore, it does not reach values near the short circuit current of the PV module, so it can not be used for all algorithms~\cite{A_New_Application_Buck_Boost_Converters_IV_Curve_PV_Modules_2007}. 

        \subsubsection{Buck converter}
            The buck converter is a step-down DC-DC converter also widely use for applications where the solar panel voltage is higher than the battery or load voltage. But in most automotive applications the area of the solar panel is limited, so the voltage is usually lower than the battery voltage, making the boost converter a more suitable choice.

            similar with the boost converter, the buck converter can emulate smaller impedances than the load impedance, and therefore, it can reach values near the open circuit voltage of the PV module~\cite{A_New_Application_Buck_Boost_Converters_IV_Curve_PV_Modules_2007}.
        ...
        \subsubsection{Buck-Boost converter}
            ...
            They are used in applications where the solar panel voltage can be both higher or lower than the battery or load voltage, also certain \ac{mppt} use it to have a wider range of operation or versatility of the used batteries.
    ...
    \subsection{Comparison between Non-isolated DC-DC converters}
        All of the non-isolated DC-DC topologies explained before have their own advantages and disadvantages. The choice of the topology will depend on the specific application, the desired performance, and the available resources. To help with this choice, Table~\ref{tab:dc-dc-comparison} shows a comparison between the most relevant characteristics of each topology. In green the best vantages of each topology are highlighted.
        ...
        \begin{adjustbox}{max width=0.95\textwidth}
            ...
            Type & \G Step-Up & \G Step-Down & \G Step-Down/Up & Step-Down/Up & Step-Down/Up & Step-Down/Up \\
            Versatility & Low & Low & Medium & \G High & \G High & \G High \\
            Inverted output & No & No & Yes & Yes & No & No \\
...
\section{Battery charging techniques}
    ...
    There are two more complex methods that use advanced control techniques to implement CC-CV, mainly the fuzzy logic and grey predict (FL-CC/CV and GP-CC/CV). These methods can adapt the charging process based on the battery's state of charge, temperature, and other factors, potentially improving charging efficiency and battery lifespan. As downside they required computational power.
...
\section{Processing unit}
    \subsection{FPGA}
        \textcolor{red}{\textbf{NOTE:} This is just a draft made with knowledge base on a "Hardware programming" course and an internship in Synopsys. This text needs to be verified with the state of art, and make some meaningful references.}

        The digital circuit of an \ac{mppt} have 4 main functions, implementation of an \acs{mppt} Algorithm, sensor aquisicision mainly done by ADCs, generate PWM signal to drive the converter circuit and, in some cases, communication.

        All of this functions can be implemented in a variety of ways. The most professional way is to use an \ac{ic}. This chips are usually design and manufactured by a team of professional engineers that dedicated their time and knowledge to develop an efficient product. But as we will discussed later in section \ref{sec:ICs}, this \acp{ic} are design and produced with a specific application in mind and depending on the application, they can be useful or not.

        Assuming that there is no IC perfect for this application, what are the options? The main options are \acp{mcu} or \acp{fpga}. There is also some less powerful options like \acp{cpld} and \ac{dsp} but usually they are to complex to work with and not powerful enough. Lets focused in \acs{fpga} for now. 
        
        In a industrial design, if a company needs an \acs{ic} with a specific specs that there is't in the market, they can design one. 
        
        The design of an \acs{ic} start with the constraints, than a design is made in a \ac{hdl} and after software testbench and verifications, the design needs to be tested in real hardware. To test this new \ac{ic} in hardware there is two main options, ask a manufactured to produce a prototype or test the circuit in an \ac{fpga}. In most real world cases, a prototype is produced because of the complexity and time constraints that can't be validated in \acs{fpga} but a \ac{ic} prototype costs thousands of euros. So every time that an \ac{fpga} can be used, they use it. 

        An \acs{fpga} is powerful design tool that implements \acs{hdl} code in real hardware, offering the possibility of cheap prototyping. 

        Comparing with \acp{mcu}, \acp{fpga} can be beneficial in terms of energy efficiency and efficiency but it is expensive, requires a lot of knowledge in \ac{hdl} and time to design. If the design will not be mass produced in the form of an \acs{ic}, it is not worth the time and money spent.

        In the other hand \acp{mcu} are easily programed, versatile, cheap, fast and reliable. Since this project is for a team of students, it needs to be easily changed for the future needs and understandable without having depth knowledge in \acp{mcu} or \acp{fpga}. With that in mind, \acp{fpga} will not be use in this project despite its advantages.
...
\section{Voltage and current sensing}
    ...
    Caso se use um transistor em vez de um diodo tem de se ter cuidado para não provocar curto circuito}


        \textcolor{red}{Operation Amplifier, Diferential Amplifier, Instromentation Amplifier, Hall effect sensors, etc.}

        \textcolor{red}{High side vs low side}
...
\label{sec:ICs}
        ...
        The MAX20801TPBA+ and SPV1040TTR, are good for IOT aplications, where batteries and solar panels are both small, but in our use case it is not suitable since it can deliver 48V. Other problem that could show up is the tracking speed, since there is no information online about it.

        The last one in the table, LT8491IUKJ\#PBF, is almost perfect. It has an wide input and output range, can do both step up and step down, it has a good efficiency, communication,  CC-CV, etc. But digging in the datasheet, there is a image showing an tracking speed of 2 seconds, which is not fast enough for this application. Additionally it uses I2C communication that would required an additional microcontroller to convert the telemetry to \acs{can}.
...
\section{Commercial \acs{mppt}s}
    As discussed in the previous section, there are some commercial available solutions for the problem in analysis. In this section is discussed the commercial solutions on layer above. This products are able to solve the problem without any extra hardware but all of them have an spec that can be improved. In table~\ref{tab:commercial-mppt-comparison} is represented a selection of \ac{mppt} commercial available with some important specs and marked in red the their worst specs.
    
    The first and most important \ac{mppt} at the table is the GVB-8-Li-CV. This converter is extremely efficient and fast, making it one of the best \acp{mppt} in the market for moving vehicles like a boat. For the past years \acs{tsb} have been using this converter in all of their boats and no problems have been encountering except the lack of customization and information. Fact that lead to this project because \acs{tsb} is a competitive team and every information about the boat is an advantage to future improvements. Additionally it does not have a programmable output voltage which make it only suitable for 48V batteries and is expensive.

    Overcome the problems of the GVB-8-Li-CV will mean success so it will used as a reference to this project.

    The Smart Solar MPPT 100/20 is one of Victron Energy solar chargers (\acs{mppt}). This brand is weal known in the motorhome community for its high efficiency, fast \acs{mpp} tracking and low cost. Their \acp{mppt} have \acs{can} and Bluetooth communication with an mobile app for data visualization. The only down side of this converter is the need of a relative large solar panel since it uses a buck converter. In \acs{tsb} application the solar panels are small and organized in groups of 2 to 3 solar panels in the same array to minimize the effects of clouds, defective solar panels or shadings. Therefore, we conclude that although this converter is excellent, it is not suitable for the application in question.

    Their are some other companies in the market but not so relevant as the two previus ones. For example the Rover Lite and MS4840N are reasonable converters but use communication protocols that not suit this project. Additionally there is only minimal information about them public available.

    There is one more converter in table \ref{tab:commercial-mppt-comparison}, the Reboost V0.2.1. This converter was built 5 years ago by student team with the automotive sector in mind and is public available. Since than it as been update over the years. All hardware and software is opensourced and can be modified by evry user to achieve the desired. The specs of this converter are near what we are looking for, but due to some overprice components and some discontinued ones this converter is not ideal for project in question. Additionally the fact of being a boost converter limitates the voltage of solar panels relative to the batterie, since the in a boost converte the output is always higher than the imput. So if in the foture \acs{tsb} decides to change from a 48V batterie to a 24V batterie, the arrays above 24V whould need be rearrange.
