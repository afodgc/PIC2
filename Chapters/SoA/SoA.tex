...
\section{MPPT Algorithms}
    ...
    \subsection{Perturb and Observe (P\&O)}
        ...
        To solve this issue, a new condition must be added to the algorithm to take into account two consecutive measures of $\Delta P$ and $\Delta V$. This way, if the sign of both $\Delta P$ are different, it means that the power change was caused by an environmental variation and not by the perturbation, so the algorithm must not change the voltage. This variation is called "two point algorithms" or "improved P\&O" \cite{bendib_survey_mppt_methods}. 
...
\section{DC-DC types}
    ...
    \subsection{Non-isolated DC-DC converters}
        ...
        \subsubsection{Buck converter}
            ...
            Similar with the boost converter, the buck converter can emulate smaller impedance than the load impedance, and therefore, it can reach values near the open circuit voltage of the PV module~\cite{A_New_Application_Buck_Boost_Converters_IV_Curve_PV_Modules_2007}.
...
\section{Switching circuit}
    \subsection{Types of transistors}
        \textcolor{red}{falar dos timpos de transistores e como dar drive deles. Caso se use um transistor em vez de um diodo tem de se ter cuidado para não provocar curto circuito}
...
\section{Processing unit}
    \subsection{FPGA}
        ...
        Comparing with \acp{mcu}, \acp{fpga} can be beneficial in terms of energy efficiency and efficiency, however it is expensive, requires a lot of knowledge in \ac{hdl} and time to design. If the design will not be mass-produced in the form of a \acs{ic}, it is not worth the time and money spent.

        In the other hand \acp{mcu} are easily programmed, versatile, cheap, fast and reliable. Since this project is for a team of students, it needs to be easily changed for the future needs and understandable without having depth knowledge in \acp{mcu} or \acp{fpga}. With that in mind, \acp{fpga} will not be use in this project despite its advantages.
...
\section{Voltage and current sensing}
    \textcolor{red}{This is just a draft based on my research writen in OneNote and converted to latex with the help of chatGPT. (It needs some work and references). The text is written as I have already decided a solution, but I did not. IT's JUST A DRAFT!!}
    ...
    Overall, shunt‑based current sensing using a differential amplifier built from an operational amplifier provides a robust, accurate and cost‑effective solution for MPPT applications. This approach offers sufficient performance without the added complexity of instrumentation amplifiers or dedicated current‑sense ICs, making it well aligned with the objectives of this project.
...
\label{sec:ICs}
        ...
        The MAX20801TPBA+ and SPV1040TTR, are good for IOT applications, where batteries and solar panels are both small, but in our use case it is not suitable since it can deliver 48V. Other problem that could show up is the tracking speed, since there is no information online about it.

        The last one in the table, LT8491IUKJ\#PBF, is almost perfect. It has an wide input and output range, can do both step up and step down, it has a good efficiency, communication,  CC-CV, etc. But digging in the data sheet, there is an image showing a tracking speed of 2 seconds, which is not fast enough for this application. Additionally, it uses I2C communication that would require an additional microcontroller to convert the telemetry to \acs{can}.
