...
\section{Processing and sensing unit}
    \subsection{Types of processing units}
        There are several processing units in the market that can be used and implemented in this project. The most common ones are \acp{mcu}, \acp{fpga} and \acp{asic}. 

        \acp{asic} are custom designed chips that are optimized for a specific application. They are usually more efficient and faster than \acp{mcu} and \acp{fpga}, but they are also more expensive and time-consuming to design and manufacture. On the other hand, there are already commercial available \acp{asic}, called \ac{ic} that overcome those problems. However, as we will discuss in section~\ref{sec:ICs}, these \acp{ic} are designed for specific applications and may not be suitable for this project.

        \ac{fpga} are programmable devices that can be configured to implement custom digital circuits. They are more flexible than \acp{ic} and can be reprogrammed if needed, but they are also more complex to design and program. These devices are separated into block internally which uses Look-Up tables (LUTs) to store all different outputs knowing the inputs. This way, complex digital circuits can be implemented by programming the LUTs and interconnections between them~\cite{231340}. The main advantage of \acp{fpga} is their parallel processing capabilities, which leads to high speed performance. However, they are usually expensive, harder to implement in terms of hardware and software and volatile, meaning that they lose their configuration when powered off~\cite{ampheo_fpga_vs_microcontroller_2025}\cite{FPGA_Implementation_MPPT}.

        Overcoming the limitations of \acp{fpga}, \acp{mcu} are widely used in embedded systems due to their ease of use, low cost, and versatility. They are programmable devices that can execute a wide range of tasks, including data acquisition, signal processing, and control. They are usually easier to program than \acp{fpga}, due to their non-volatile flash memory, and have a large community of developers and resources available. However, they are generally slower in parallel processing than \acp{fpga} due to their sequential processing architecture~\cite{ampheo_fpga_vs_microcontroller_2025}. Overall, \acp{mcu} are cheaper and required less additional hardware. Additionally, they are easier to program and understand, making them less time-consuming for development and debugging. 

        For this application, a \ac{mcu} will be used due to the importance of a low cost and ease of use solution. Additionally, the processing power of modern \acp{mcu} is sufficient to handle the requirements of the \ac{mppt} algorithm and battery charging technique.
